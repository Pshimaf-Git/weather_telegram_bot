...
func (s *Server) StartComahd(u *tgbotapi.Update) error {
	if err := s.SendMsg(u, fmt.Sprintf(formatHelloMsg, u.Message.From.FirstName)); err != nil {
		s.logger.Error("send hello message", zap.String("whom", u.Message.From.UserName), zap.Error(err))
...
func (s *Server) GetWeather(u *tgbotapi.Update) error {
	...
	if err != nil {
		s.logger.Error("get weather from memory storage", zap.String("city", city), zap.Error(err))
		return err
	}

	if ok {
		if err := s.SendMsg(u, formatMsg(weather, u.Message.From.LanguageCode)); err != nil {
			s.logger.Error("send weather message", zap.String("whom", u.Message.From.UserName), zap.Error(err))

			return fmt.Errorf("send weather: %w", err)
		}
	}
	...
	if !ok {
		...
		if err != nil {
			s.logger.Error("get weather from foreighn api", zap.String("city", city), zap.Error(err))
			return err
		}
		...
		if err := s.memo.Put(weather, u.Message.From.LanguageCode); err != nil {
			s.logger.Error("put weather in memory storage", zap.Any("weather", weather), zap.Error(err))
			// Don't return err
		}
	}
